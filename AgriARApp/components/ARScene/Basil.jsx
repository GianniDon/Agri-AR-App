...
// Pre-load assets
const modelAsset = Asset.fromModule(require('../../assets/models/source/basil.obj'));
const textureAssets = {
  basil: Asset.fromModule(require('../../assets/models/textures/basil_texture/basil.jpeg'))
};

const SCREEN_HEIGHT = Dimensions.get('window').height;

const BasilScene = ({ showAR, onStopAR,navigation }) => {
   // States
 ...
 const ensureLocalAsset = async (asset) => {
  ...
  const loadAssets = async () => {
    try {
      ...
      const basilUri = await ensureLocalAsset(textureAssets.basil);
      setLoadingProgress(70);

      return {
        modelUri,
        basilTextUri: basilUri
      }
    } catch (error) {
      setError(`Errore nel caricamento degli asset: ${error.message}`);
      throw error
    }
  };
  ...
  const onContextCreate = async (gl) => {
    try {
      ...
      const { modelUri, basilTextUri } = await loadAssets();

    
      const TextureBasil = new TextureLoader().load(basilTextUri);
      TextureBasil.encoding = sRGBEncoding;
      ...
      const materialBasil = new MeshStandardMaterial({
        map: TextureBasil,
      });

      object.traverse((child) => {
        if (child.isMesh) {
          if (child.name.includes('Texture_1_jpeg')) {
            child.material = materialBasil;
          } 
           else {
            child.material = materialBasil; // default material
          }
        }
      });
  ...
  const fetchPlantData = async() => {
    try {
      ...
      const response = await fetch(
        `https://openfarm.cc/api/v1/crops?filter=basil`,
        {
          method : 'GET',
          headers:{
  ...
  const fetchNutritionData = async () => {
    try {
      const response = await fetch(
        'https://world.openfoodfacts.org/api/v0/product/5410188030200.json'
      );
 ...
 return (
     <View style={styles.container}>
       ...
       {showInfo && (
         <>
           <View
             style={[styles.glContainer, { height: isInfoExpanded ? '35%' : '60%' ,
              minHeight: 150
              }]}
                 ...
                 farmData && (
                   <View style={styles.infoSection}>
                     <View style={styles.headerContainer}>
                       <Text style={styles.mainTitle}>Basilico</Text>
