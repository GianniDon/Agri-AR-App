...
const modelAsset = Asset.fromModule(require('../../assets/models/source/carrot.obj'));
const textureAssets = {
  carrot: Asset.fromModule(require('../../assets/models/textures/carrot_texture/carrot.jpeg')),
  internal: Asset.fromModule(require('../../assets/models/textures/carrot_texture/internal.jpeg')),
};

const SCREEN_HEIGHT = Dimensions.get('window').height;

const CarrotScene = ({ showAR, onStopAR, navigation }) => {
  // States
  ...
  const loadAssets = async () => {
    try {
      ...
      const carrotUri = await ensureLocalAsset(textureAssets.carrot);
      setLoadingProgress(50);
      const internalUri = await ensureLocalAsset(textureAssets.internal);
      setLoadingProgress(70);
        return { modelUri, 
            carrotTextureUri: carrotUri, 
            internalTextureUri: internalUri };
    } catch (error) {
  ...
  const onContextCreate = async (gl) => {
    try {
      ...
      const { modelUri, carrotTextureUri, internalTextureUri } = await loadAssets();

      const textureCarrot = new TextureLoader().load(carrotTextureUri);
      textureCarrot.encoding = sRGBEncoding;

      const textureInternal = new TextureLoader().load(internalTextureUri);
      textureInternal.encoding = sRGBEncoding;
      ...
      const materialCarrot = new MeshStandardMaterial({
        map: textureCarrot,
      });

      const materialInternal = new MeshStandardMaterial({
        map: textureInternal,
      });
      ...
      object.traverse((child) => {
        if (child.isMesh) {
          if (child.name.includes('mmGroup0')) {
            child.material = materialCarrot;
          } else if (child.name.includes('mmGroup0')) {
            child.material = materialInternal;
          } else {
            child.material = materialLeaves; // default material
          }
        }
      });
  ...
  const fetchNutritionData = async () => {
    try {
      const response = await fetch(
        'https://world.openfoodfacts.org/api/v0/product/8011433010025.json'
      );
      ...
      if (data.status === 1) {
        ...
        setFoodData({
          productName: data.product.product_name || 'Carota',
  ...
  const fetchPlantData = async () => {
    try {
      setIsLoading(true);
      const response = await fetch(
        `https://openfarm.cc/api/v1/crops?filter=carrot`,
        {
          method: 'GET',
          headers: {
                ...
                farmData && (
                  <View style={styles.infoSection}>
                    <View style={styles.headerContainer}>
                      <Text style={styles.mainTitle}>Carota</Text>
