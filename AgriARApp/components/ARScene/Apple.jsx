...
const modelAsset = Asset.fromModule(require('../../assets/models/source/apple2.obj'));
const textureAssets = {
  apple: Asset.fromModule(require('../../assets/models/textures/apple_texture/apple2.jpeg')),
  internal: Asset.fromModule(require('../../assets/models/textures/apple_texture/internal_ground_ao_texture.jpeg')),
};

const SCREEN_HEIGHT = Dimensions.get('window').height;

const AppleScene = ({ showAR, onStopAR, navigation }) => {
  // States
  ...
  const loadAssets = async () => {
    try {
      ...
      const appleUri = await ensureLocalAsset(textureAssets.apple);
      ...
      return { 
        modelUri, 
       appletextureUri: appleUri,
       interalTextureUri: internalUri
      };
    } catch (error) {
  ...
  const onContextCreate = async (gl) => {
    try {
      ...
      const { modelUri, appletextureUri, interalTextureUri } = await loadAssets();

      const textureApple = new TextureLoader().load(appletextureUri);
      textureApple.encoding = sRGBEncoding;

      const textureInternal = new TextureLoader().load(interalTextureUri);
      ...
      const materialApple = new MeshStandardMaterial({
        map: textureApple,
      });
      ...
      object.traverse((child) => {
        if (child.isMesh) {
          if (child.name.includes('default')) {
            child.material = materialApple;
          } else {
            child.material = materialApple; // default material
          }
        }
      });
  ...
  const fetchNutritionData = async () => {
    try {
      const response = await fetch(
        'https://world.openfoodfacts.org/api/v0/product/8025920000239.json'
      );
      ...
      if (data.status === 1) {
        ...
        setFoodData({
          productName: data.product.product_name || 'Mela',
  ...
  const fetchPlantData = async () => {
    try {
      setIsLoading(true);
      const response = await fetch(
        `https://openfarm.cc/api/v1/crops?filter=apple`,
        {
          method: 'GET',
          headers: {
                ...
                farmData && (
                  <View style={styles.infoSection}>
                    <View style={styles.headerContainer}>
                      <Text style={styles.mainTitle}>Mela</Text>
